...
public class Premium extends Induk{
    public Premium(){}
    public Premium(String Nama, String NIK, String No_HP, String Alamat){
    ...
    public void TampilkanInfo(){
        ...
        for (int i = 0; i < Fasilitas.length; i++) {
                System.out.println("\t"+Fasilitas[i] + " : " + JumlahFasilitas[i]);
        }
    ...
    public void CountFasilitasAndMakan(String JenisKamar, int n) {
        if(JenisKamar.equals("Suite")){
            ...
            JumlahMakan[1] += 6*n;
            JumlahMakan[2] += 6*n;
            JumlahFasilitas[0]   += 6*n;
            JumlahFasilitas[1]   += 6*n;
            JumlahFasilitas[2]   += 6*n;
        }
        else if(JenisKamar.equals("Double")){
            JumlahMakan[0] += 2*n;
            JumlahMakan[1] += 2*n;
            JumlahMakan[2] += 2*n;
            JumlahFasilitas[0]   += 2*n;
            JumlahFasilitas[1]   += 2*n;
            JumlahFasilitas[2]   += 2*n;
        }
        else{
            JumlahMakan[0] += 1*n;
            JumlahMakan[1] += 1*n;
            JumlahMakan[2] += 1*n;
            JumlahFasilitas[0]   += 1*n;
            JumlahFasilitas[1]   += 1*n;
            JumlahFasilitas[2]   += 1*n;
        }
    }
